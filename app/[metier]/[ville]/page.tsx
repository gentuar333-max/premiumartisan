...
// Next 14/15 params: mund të jetë Promise ose objekt normal
async function getVille(params: unknown): Promise<string> {
  if (params && typeof params === "object" && "then" in (params as any)) {
    const resolved = await (params as Promise<{ ville: string }>);
    return resolved.ville;
  }
  return (params as { ville: string }).ville;
}
...
export async function generateMetadata({
  ...
  const ville = await getVille(params);
...
export default async function Page({
  ...
  const faqJsonLd = {
    ...
    mainEntity: [
      ...
      },
      {
        ...
        acceptedAnswer: {
          "@type": "Answer",
          text: "Oui. La demande de devis est gratuite et sans engagement. Vous comparez librement les propositions.",
        },
      },
      {
        "@type": "Question",
        ...
        acceptedAnswer: {
          "@type": "Answer",
          text: "Votre demande est diffusée à 4 artisans maximum afin de garantir des réponses rapides et de qualité.",
        },
      },
    ],
  };
  ...
  return (
    <main style={{ maxWidth: 920, margin: "0 auto", padding: "32px 20px" }}>
      ...
      <p style={{ fontSize: 18, lineHeight: 1.6, maxWidth: 680 }}>
        Recevez jusqu’à 4 devis peinture gratuits à {cityName}, sans engagement. Décrivez votre projet en quelques secondes et soyez contacté par des artisans qualifiés proches de chez vous.
      </p>

      <div style={{ marginTop: 24, padding: 24, borderRadius: 16, border: "1px solid #e5e7eb", background: "#ffffff" }}>
      ...
      <section style={{ marginTop: 40, maxWidth: 720 }}>
        ...
        <p style={{ lineHeight: 1.8 }}>
          Que ce soit pour la peinture intérieure, la rénovation d’appartement ou un chantier complet, notre plateforme vous permet de recevoir des devis adaptés à votre projet à {cityName}.
        </p>
        ...
        <p style={{ lineHeight: 1.8 }}>
          La demande de devis est 100% gratuite et sans engagement. Vous restez libre de choisir l’artisan. Votre projet est partagé avec un maximum de 4 artisans.
        </p>
      </section>

      <section style={{ marginTop: 40, maxWidth: 720 }}>
        <h2 style={{ fontSize: 24, fontWeight: 700 }}>Questions fréquentes</h2>

        <h3 style={{ marginTop: 16 }}>Est-ce que le devis peinture est vraiment gratuit ?</h3>
        ...
        <p>La majorité des clients reçoivent des réponses en moins de 24 heures selon la zone et la complexité.</p>
      </section>
    </main>
  );
}