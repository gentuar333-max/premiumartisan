  ...
  const dots = [0, 1, 2].map((i) => (
    <span
      ...
      className={`h-1.5 w-1.5 rounded-full ${
        i < count ? "bg-orange-400" : "bg-slate-600"
      }`}
    />
  ));
  return (
...
const ProjectCard = memo(function ProjectCard({
  ...
  const categoryRaw = (p.category_details ||
    p.category ||
    categoryLabel ||
    "Peinture") as string;
  ...
  const name =
    nameParts.length >= 2
      ...
      : rawName;
  const zoneLabel =
    p.location?.trim() || `Département ${p.postal_prefix ?? ""}`;
  ...
  return (
    <div
      className={`group overflow-hidden rounded-2xl border bg-white shadow-sm transition hover:shadow-md ${
        isLockedForMe ? "border-slate-300 opacity-80" : "border-slate-200"
      }`}
      onMouseEnter={() =>
        p.id && p.id !== "undefined" && onPrefetch(p.id)
      }
    >
        ...
        {isLockedForMe && (
          <div className="absolute inset-x-0 top-0 flex items-center justify-center gap-1.5 bg-slate-900/80 px-3 py-2 backdrop-blur-sm">
            ...
            <span className="text-[12px] font-semibold text-slate-200">
              Projet réservé — déjà attribué à un artisan
            </span>
          </div>
        )}
        ...
        <div className="flex items-start justify-between gap-3">
          <h3
            className={`text-lg font-semibold leading-tight ${
              isLockedForMe ? "text-slate-400" : "text-rose-600"
            }`}
          >
            {name}
          </h3>
          <div
            className={`whitespace-nowrap text-sm font-semibold ${
              isLockedForMe ? "text-slate-400" : "text-emerald-600"
            }`}
          >
            ...
            <div className="flex items-start gap-2">
              ...
              <div className="min-w-0">
                ...
                <p className="mt-0.5 text-[11px] text-amber-700">
                  Soyez le seul à recevoir ce projet — bloquez les autres
                  artisans
                </p>
              </div>
            </div>
        ...
        <div className="mt-3 flex flex-col gap-2">
          <div className="flex flex-wrap items-center gap-2">
            {p.id && p.id !== "undefined" ? (
              <Link
                ...
                className="shrink-0 text-sm text-slate-600 underline underline-offset-4 hover:text-slate-900"
              >
                Voir le projet
              </Link>
            ) : (
            ...
            {!isLockedForMe && !isUnlocked && !isFull && (
              <div className="ml-auto flex items-center gap-2">
                <button
                  type="button"
                  onClick={() =>
                    p.id && p.id !== "undefined" && onUnlockClick(p.id)
                  }
            ...
            {!isLockedForMe && isUnlocked && (
              <div className="ml-auto flex items-center gap-2">
                {waE164 ? (
                  <a
                    href={`https://wa.me/${waE164}?text=${encodeURIComponent(
                      "Bonjour, j'ai vu votre projet sur PremiumArtisan"
                    )}`}
                    target="_blank"
                    rel="noreferrer"
          ...
          {isFirstSlot && !isUnlocked && !isLockedForMe && (
            <button
              type="button"
              onClick={() =>
                p.id && p.id !== "undefined" && onUnlockExclusive(p.id)
              }
              disabled={!p.id || p.id === "undefined" || unlocking}
...
function FilterPanel({
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setBudgetMin(initialBudgetMin);
      setBudgetMax(initialBudgetMax);
    }
  ...
  function handleApply() {
    ...
    if (prefix) params.set("cp", prefix);
    else if (zone.trim()) params.set("cp", zone.trim());
  ...
  return (
    <>
      ...
      />
      <div
        className="fixed left-0 top-0 z-50 flex h-full w-[85vw] max-w-[340px] flex-col bg-white shadow-2xl"
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4">
          <div className="flex flex-col divide-y divide-slate-100">
            <div className="pb-5">
              <p className="mb-2 text-[12px] font-bold uppercase tracking-widest text-slate-500">
                Zone / Département
              </p>
            ...
            <div className="py-4">
              <p className="mb-1 text-[12px] font-bold uppercase tracking-widest text-slate-500">
                Catégorie
              </p>
            ...
            <div className="py-4">
              ...
              {STATUTS.map(({ key, label }) => (
                <CheckItem
                  ...
                  label={label}
                  checked={statut === key}
                  onChange={() => setStatut(statut === key ? "" : key)}
                />
              ))}
            </div>
            <div className="py-4">
              <p className="mb-3 text-[12px] font-bold uppercase tracking-widest text-slate-500">
                Budget
              </p>
              <div className="flex gap-3">
                {[
                  { value: budgetMin, onChange: setBudgetMin, options: BUDGET_OPTIONS },
                  {
                    ...
                    options: BUDGET_OPTIONS.map((o, i) =>
                      i === 0 ? { ...o, label: "€ No Max" } : o
                    ),
                  },
            ...
            <div className="py-4">
              ...
              {SORTS.map(({ key, label }) => (
                <CheckItem
                  key={key}
                  label={label}
                  checked={localSort === key}
                  onChange={() => setLocalSort(key)}
                />
              ))}
            </div>
          </div>
        ...
        <div className="shrink-0 border-t border-slate-200 bg-white px-6 py-4">
          <div className="flex items-center gap-4">
            ...
            >
              ...
              {activeCount > 0 && (
                <span className="ml-2 inline-flex h-5 w-5 items-center justify-center rounded-full bg-white text-[11px] font-bold text-slate-900">
                  {activeCount}
                </span>
              )}
            </button>
...
export function DashboardShell({
  ...
  const [unlockStatuses, setUnlockStatuses] = useState<
    Record<string, { status: string | null; conversation_id: string | null }>
  >({});
  const [unlockedContacts, setUnlockedContacts] = useState<
    Record<string, { phone: string | null }>
  >({});
  const [projectUnlockCounts, setProjectUnlockCounts] = useState<
    Record<string, number>
  >({});
  const [projectIsLocked, setProjectIsLocked] = useState<
    Record<string, boolean>
  >({});
  const [unlockingProjectId, setUnlockingProjectId] = useState<string | null>(
    null
  );
  ...
  const currentCats = (searchParams.get("cat") ?? "")
    .split(",")
    .filter(Boolean);
  ...
  const activeFilterCount =
    currentCats.length +
    (currentStatut ? 1 : 0) +
    (currentBudgetMin ? 1 : 0) +
    (currentBudgetMax ? 1 : 0);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        const sm: Record<
          string,
          { status: string | null; conversation_id: string | null }
        > = {};
        ...
        if (json.unlock_counts)
          setProjectUnlockCounts(
            json.unlock_counts as Record<string, number>
          );
        if (json.is_locked)
          setProjectIsLocked(json.is_locked as Record<string, boolean>);
      } catch {
        if (!cancelled) setUnlockStatuses({});
      }
    };
    void load();
    return () => {
  ...
  const prefetchProject = useCallback(
    (id: string) => {
      router.prefetch(`/artisan/project/${id}`);
    },
    [router]
  );
  ...
  useEffect(() => {
    projects
      ...
      .forEach((p) => prefetchProject(p.id!));
  ...
  const onUnlockClick = useCallback(
    async (projectId: string) => {
      ...
      try {
        ...
        if (!res.ok || !json?.ok) {
          setToast({
            msg: json?.error || "Impossible de récupérer le numéro.",
            type: "error",
          });
          return;
        }
        ...
        setUnlockedContacts((prev) => ({
          ...prev,
          [projectId]: { phone: json?.contacts?.[projectId]?.phone ?? null },
        }));
        setProjectUnlockCounts((prev) => ({
          ...prev,
  ...
  const onUnlockExclusive = useCallback(
    async (projectId: string) => {
      if (!currentUser) {
        router.push(`/artisan/login?redirect=/artisan/dashboard`);
        return;
      ...
      try {
        ...
        if (!res.ok || !json?.ok) {
          ...
          setToast({
            msg: json?.error || "Erreur lors de la réservation exclusive.",
            type: "error",
          });
          return;
        }
  ...
  useEffect(() => {
    ...
    if (unlockToast === "success") {
      setToast({
        msg: "Paiement confirmé ! Numéro débloqué.",
        type: "success",
      });
  ...
  return (
    <div className="min-h-screen bg-[#f0f2f5]">
      ...
      <header className="sticky top-0 z-30 bg-[#eaecef] border-b border-[#d5d8dc] shadow-sm">
        <div className="mx-auto flex h-[52px] max-w-7xl items-center gap-2 px-4 sm:px-6">
          ...
          <button type="button" onClick={() => setFilterOpen(true)}
            ...
            {activeFilterCount > 0 && (
              <span className="flex h-4 w-4 items-center justify-center rounded-full bg-slate-500 text-[9px] font-bold text-white">
                {activeFilterCount}
              </span>
            )}
          </button>

          {/* Réceptionniste IA — style normal */}
          <Link
            href="/artisan/receptionist"
            target="_blank"
            rel="noopener noreferrer"
            className="shrink-0 flex items-center gap-1.5 px-2.5 py-1.5 text-sm text-slate-600 transition hover:text-slate-900 hover:bg-slate-200 rounded-lg whitespace-nowrap"
          >
            <span className="text-[13px]">🎙</span>
            <span className="hidden sm:inline">Réceptionniste IA</span>
          </Link>

          {/* Search */}
          <div className="relative flex-1 min-w-[80px] max-w-sm mx-2">
          ...
          <button
            ...
            aria-label="Menu"
            className="shrink-0 ml-auto flex h-8 w-8 flex-col items-center justify-center gap-[3.5px] rounded-lg border border-slate-200 bg-white transition hover:bg-slate-50"
          >
      ...
      <FilterPanel
        ...
        initialCp={cp}
        initialSort={sort}
        initialCats={currentCats}
        initialStatut={currentStatut}
        initialBudgetMin={currentBudgetMin}
        initialBudgetMax={currentBudgetMax}
          ...
          <aside className="fixed right-0 top-0 z-50 flex h-full w-[280px] max-w-[80vw] flex-col bg-white shadow-2xl">
            ...
            <div className="flex flex-1 flex-col gap-3 p-5">
              ...
              <Link href="/artisan/receptionist"
                ...
                className="w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-center text-sm font-semibold text-slate-700 transition hover:bg-slate-50 flex items-center justify-center gap-2"
                onClick={() => setMenuOpen(false)}>
                🎙 Réceptionniste IA
              </Link>
              ...
              {activeFilterCount > 0 && (
                <button type="button"
                  onClick={() => { handleFilterReset(); setMenuOpen(false); }}
      ...
      <div className="mx-auto max-w-7xl px-4 py-5 sm:px-6">
        {newHit && (
          <div className="mb-4 flex items-center justify-between rounded-xl border border-blue-200 bg-blue-50 p-4 text-[13px] text-blue-900">
            <div>
              <b>Nouveau projet dans votre zone !</b> La liste a été mise à jour.
            </div>
            <button
              onClick={() => setNewHit(null)}
              className="ml-4 rounded-lg border border-blue-200 bg-white px-3 py-1.5 text-sm font-semibold text-blue-800 hover:bg-blue-100"
            >
              OK
            </button>
          </div>
        ...
        {toast && (
          <div className={`mb-4 rounded-xl border p-3 text-[13px] ${toastColors[toast.type]}`}>
            {toast.msg}
          </div>
        )}
        {error && (
          ...
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {projects.map((p) => (
              <ProjectCard
                ...
                onUnlockExclusive={onUnlockExclusive}
