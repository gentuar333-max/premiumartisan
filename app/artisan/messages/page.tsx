...
export default function ArtisanMessagesPage() {
  ...
  const loadConversations = useCallback(async () => {
    ...
    try {
      ...
      if (initialProjectId) {
        const byProject = rows.find((r) => r.project_id === initialProjectId);
        if (byProject) {
          setActiveConversationId(byProject.id);
          return;
        }
      }
  ...
  useEffect(() => {
    if (toastParam === "unlock_success") {
      setToast("Contact débloqué ✅");
    }
  }, [toastParam]);
  ...
  useEffect(() => {
    ...
    const channel = sb
      .channel(`messages-artisan-${activeConversationId}`)
      .on(
        "postgres_changes",
        {
          event: "INSERT",
      ...
      <div className="mx-auto grid max-w-6xl grid-cols-1 gap-4 md:grid-cols-[320px_1fr]">
        ...
        <section className="flex min-h-[560px] flex-col rounded-2xl border border-slate-200 bg-white shadow-sm">
          <div className="border-b border-slate-200 px-4 py-3">
            ...
            {activeConversation && (
              <div className="text-xs text-slate-500">
                Client: {activeConversation.counterpart_name || "Client"}
              </div>
            )}
          </div>
