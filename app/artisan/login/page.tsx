...
// Dev: NEXT_PUBLIC_DEV_BYPASS_UNLOCK=true → min 8 chars, no complexity.
// If Supabase rejects weak passwords, set Auth > Settings > Password: min 6 chars in the Supabase dashboard for dev project.
...
function isPasswordValid(pwd: string): boolean {
  ...
  if (DEV_MODE) return true; // dev: 8 chars only
  const strength = passwordStrength(pwd);
  return strength.score >= 2; // prod: min length + complexity (e.g. digit, upper+lower, or special)
}
...
/** Replace Supabase rate-limit countdown messages with a generic message. Never show "wait XX seconds". */
...
export default function ArtisanLoginPage() {
  ...
  const redirectTo = searchParams.get("redirectTo") ?? "/artisan/dashboard";
    ...
    try {
      if (mode === "signup") {
        ...
        if (signUpData?.session && signUpData?.user) {
          const apiUrl = typeof window !== "undefined" ? `${window.location.origin}/api/auth/set-role` : "/api/auth/set-role";
          let res: Response;
          try {
            ...
            if (DEV) console.error("[artisan/signup] fetch failed:", apiUrl, fetchErr);
          ...
          if (!res.ok) {
            ...
            if (DEV) console.error("[artisan/signup] set-role failed:", res.status, apiUrl, json);
          ...
          <form onSubmit={handleSubmit} className="mt-6 space-y-4">
            ...
            {mode === "signup" && (
              <>
                ...
                <div>
                  <label htmlFor="metier" className="block text-sm font-medium text-slate-700">
                    Métier (catégorie principale)
                  </label>
                  <select
                    id="metier"
                    value={metier}
              ...
              {mode === "signup" && password && !DEV_MODE && (
                <div className="mt-1.5 flex items-center gap-2">
                  ...
                  <span className="text-xs text-slate-500">
                    {pwStrength.label} {password.length < minLen && `(${minLen} caractères min)`}
          ...
          <div className="mt-6 space-y-4">
            ...
            <form onSubmit={handleForgotPassword} className="space-y-4">
              ...
              <div className="flex gap-2">
                ...
                <button
                  type="button"
                  onClick={() => { setShowForgotForm(false); setError(null); }}
                  className="rounded-lg border border-slate-300 px-4 py-2.5 text-sm font-medium text-slate-700 hover:bg-slate-50"
                >
                  Annuler
                </button>
