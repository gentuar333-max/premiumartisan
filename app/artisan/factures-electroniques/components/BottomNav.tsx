...
export default function BottomNav({ active }: { active: ActiveTab }) {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50"
      style={{ background: 'rgba(255,255,255,0.95)', backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)', borderTop: '1px solid rgba(230,223,214,0.5)', height: 64 }}>
        ...
        {TABS.map((tab) => {
          ...
          return (
            <button key={tab.key} onClick={() => router.push(tab.path)}
              ...
              style={{ width: 64, height: 56, background: 'none', border: 'none', cursor: 'pointer', WebkitTapHighlightColor: 'transparent' }}>
              {isActive && (
                <div className="rounded-full" style={{ width: 4, height: 4, backgroundColor: '#E87E1A', marginBottom: 2 }} />
              )}
