...
function ClientMessagesContent() {
  ...
  const loadConversations = useCallback(async () => {
    ...
    try {
      ...
      if (initialConversationId && rows.some((r) => r.id === initialConversationId)) { setActiveConversationId(initialConversationId); }
      else if (!activeConversationId && rows.length > 0) { setActiveConversationId(rows[0].id); }
  ...
  useEffect(() => {
    ...
    const channel = sb.channel(`messages-client-${activeConversationId}`)
    ...
    <main className="min-h-screen bg-slate-50 p-4 md:p-6">
      <div className="mx-auto grid max-w-6xl grid-cols-1 gap-4 md:grid-cols-[320px_1fr]">
        <aside className="rounded-2xl border border-slate-200 bg-white p-3 shadow-sm">
          ...
          {loadingConversations ? <div className="text-sm text-slate-500">Chargement...</div>
            : conversations.length === 0 ? <div className="text-sm text-slate-500">Aucune conversation.</div>
        ...
        <section className="flex min-h-[560px] flex-col rounded-2xl border border-slate-200 bg-white shadow-sm">
          <div className="border-b border-slate-200 px-4 py-3">
            ...
            {activeConversation && <div className="text-xs text-slate-500">Interlocuteur: {activeConversation.counterpart_name || "Artisan"}</div>}
          </div>
