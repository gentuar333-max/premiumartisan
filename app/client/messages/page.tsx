...
export default function ClientMessagesPage() {
  ...
  const loadConversations = useCallback(async () => {
    ...
    try {
      const res = await fetch("/api/messages/conversations?role=client", { cache: "no-store" });
      const json = await res.json().catch(() => null);
      if (!res.ok || !json?.ok) {
        setConversations([]);
        setToast(json?.error || "Impossible de charger les conversations.");
        return;
      }
      ...
      if (initialConversationId && rows.some((r) => r.id === initialConversationId)) {
  ...
  useEffect(() => {
    ...
    const channel = sb
      .channel(`messages-client-${activeConversationId}`)
      .on(
        "postgres_changes",
        {
          event: "INSERT",
    ...
    <main className="min-h-screen bg-slate-50 p-4 md:p-6">
      <div className="mx-auto grid max-w-6xl grid-cols-1 gap-4 md:grid-cols-[320px_1fr]">
        <aside className="rounded-2xl border border-slate-200 bg-white p-3 shadow-sm">
          <h1 className="mb-3 text-lg font-semibold text-slate-900">Messages client</h1>
              ...
              {conversations.map((c) => (
                ...
                >
                  ...
                  <div className="text-xs text-slate-500">
                    {c.project?.location || `Département ${c.project?.postal_prefix || "-"}`}
                  </div>
                  <div className="mt-1 line-clamp-1 text-xs text-slate-600">
                    {c.last_message_text || "Aucun message"}
                  </div>
                </button>
              ))}
            </div>
        ...
        <section className="flex min-h-[560px] flex-col rounded-2xl border border-slate-200 bg-white shadow-sm">
          <div className="border-b border-slate-200 px-4 py-3">
            <div className="text-sm font-semibold text-slate-900">
              {activeConversation ? formatConversationTitle(activeConversation) : "Sélectionnez une conversation"}
            </div>
            {activeConversation && (
              <div className="text-xs text-slate-500">Interlocuteur: {activeConversation.counterpart_name || "Artisan"}</div>
            )}
          </div>
          ...
          <div className="flex-1 space-y-3 overflow-y-auto px-4 py-4">
            {loadingMessages ? (
              ...
              <div className="text-sm text-slate-500">Démarrez la discussion.</div>
            ) : (
              messages.map((m) => {
      ...
      {toast && (
        <div className="fixed bottom-5 right-5 rounded-xl border border-rose-200 bg-rose-50 px-4 py-2 text-sm text-rose-900 shadow">
          {toast}
        </div>
      )}
    </main>
