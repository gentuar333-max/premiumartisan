...
function SignaturePad({ onSign }: { onSign: (dataUrl: string) => void }) {
  ...
  return (
    <div>
      ...
      {signed && <button type="button" onClick={clear} className="mt-1.5 text-[11px] text-[#be123c] hover:underline">Effacer</button>}
    </div>
  );
}
...
export default function DevisForm() {
  ...
  // Génère les valeurs dynamiques côté client uniquement (évite hydration mismatch)
  // Init valeurs dynamiques côté client
  useEffect(() => {
    ...
    setData(d => ({
      ...d,
      numero: d.numero || `DEV-${year}-${rand}`,
      dateValidite: d.dateValidite || validite,
      dateEmission: d.dateEmission || emission,
    }));
  }, []);

  // Auto-statut: Expiré si dateValidite dépassée ET statut = brouillon/envoye
  useEffect(() => {
    if (!data.dateValidite) return;
    if (data.statut === "accepte" || data.statut === "refuse" || data.statut === "expire") return;
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    const validite = new Date(data.dateValidite);
    validite.setHours(0, 0, 0, 0);
    if (validite < today) {
      setData(d => ({ ...d, statut: "expire" }));
    }
  }, [data.dateValidite, data.statut]);
  ...
  function buildPayload() {
    const sousTotal = data.lignes.reduce((s, l) => s + l.quantite * l.prixUnitaire, 0);
    const tvaMontant = sousTotal * (data.tvaRate / 100);
    ...
    return {
      ...
      date_debut_travaux: data.dateDebutTravaux || null,
      duree_estimee:      data.dureeEstimee,
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      let res: Response;
      if (devisId) {
        res = await fetch("/api/artisan/devis", {
          method: "PUT",
      ...
      if (!res.ok) { setSaveErr(json.error || "Erreur sauvegarde"); return; }
      if (!devisId) setDevisId(json.devis?.id ?? null);
      setSaved(true);
  ...
  const handleSendEmail = async () => {
    ...
    setSaveErr(null);
    try {
      // Sauvegarde d'abord si pas encore sauvegardé
      let id = devisId;
      if (!id) {
        const res = await fetch("/api/artisan/devis", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(buildPayload()),
        });
        const json = await res.json();
        if (!res.ok) { setSaveErr(json.error || "Erreur sauvegarde"); setSending(false); return; }
        id = json.devis?.id ?? null;
        if (id) setDevisId(id);
      }
      if (!id) { setSaveErr("Impossible de sauvegarder le devis"); setSending(false); return; }

      // Envoie l'email
      const sendRes = await fetch("/api/artisan/devis/send", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ devisId: id }),
      });
      const sendJson = await sendRes.json();
      if (!sendRes.ok) { setSaveErr(sendJson.error || "Erreur envoi email"); setSending(false); return; }
    ...
    <div className="min-h-screen" style={{ background: "#fdf2f5", fontFamily: "'DM Sans', system-ui, sans-serif" }}>
      ...
      <div className="sticky top-0 z-40 border-b border-[#e8d0d8] bg-white/95 backdrop-blur-sm px-5 py-3 flex items-center justify-between gap-3 flex-wrap">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <div className="flex rounded-xl border border-[#e8d0d8] overflow-hidden">
            <button type="button" onClick={() => setTab("form")} className={`px-3 py-1.5 text-[12px] font-semibold transition ${tab === "form" ? "bg-[#2a0a14] text-white" : "text-[#6a3a4a]"}`}>Formulaire</button>
            <button type="button" onClick={() => setTab("preview")} className={`px-3 py-1.5 text-[12px] font-semibold transition ${tab === "preview" ? "bg-[#2a0a14] text-white" : "text-[#6a3a4a]"}`}>Aperçu</button>
          </div>
          <button type="button" onClick={handleConvertirFacture} className="px-3 py-2 rounded-xl border border-[#e8d0d8] text-[12px] font-semibold text-[#6a3a4a] hover:border-[#be123c] hover:text-[#be123c] transition">🧾 → Facture</button>
          <button type="button" onClick={handleSendEmail} disabled={sending || sentOk} className={`px-3 py-2 rounded-xl text-[12px] font-semibold border transition ${sentOk ? "bg-green-50 border-green-300 text-green-700" : "border-[#e8d0d8] text-[#6a3a4a] hover:border-[#be123c]"}`}>
            {sentOk ? "✓ Envoyé !" : sending ? "Envoi…" : "Envoyer au client"}
          </button>
          <button type="button" onClick={handleSave} className="px-3 py-2 rounded-xl border border-[#e8d0d8] text-[12px] font-semibold text-[#6a3a4a] hover:border-[#be123c] transition">
            {saved ? "✓ Sauvegardé" : "Sauvegarder"}
          </button>
          <button type="button" onClick={handlePrint} className="px-4 py-2 rounded-xl bg-[#be123c] text-white text-[12px] font-bold hover:bg-[#e11d48] transition shadow-[0_4px_14px_rgba(190,18,60,0.3)]">📄 PDF</button>
          {saveErr && <span className="text-[11px] text-red-600 font-medium">{saveErr}</span>}
        </div>
      </div>
      ...
      <div className="max-w-[860px] mx-auto px-4 py-5">
        ...
        {/* FORMULAIRE */}
        <div className={`space-y-3 ${tab === "preview" ? "hidden" : ""}`}>
          ...
          <div className="rounded-2xl border border-[#e8d0d8] bg-white p-5">
            <Section title="Statut du devis">
              <div className="flex gap-2 flex-wrap">
                {STATUTS.map((s) => (
                  <button type="button" key={s.value} onClick={() => setData((d) => ({ ...d, statut: s.value }))}
                    className="px-3 py-1.5 rounded-full text-[12px] font-bold border-2 transition cursor-pointer"
          ...
          <div className="rounded-2xl border border-[#e8d0d8] bg-white p-5">
            <Section title="Vos informations (Artisan)">
              ...
              <div className="flex items-center gap-3 mb-2">
                ...
                <div>
                  <button type="button" onClick={() => logoRef.current?.click()} className="text-[12px] font-semibold text-[#be123c] hover:underline block">
          ...
          <div className="rounded-2xl border border-[#e8d0d8] bg-white p-5">
            <div className="flex items-center justify-between mb-4">
              ...
              <button type="button" onClick={addLigne} className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-[#fdf2f5] border border-[#e8d0d8] text-[12px] font-semibold text-[#be123c] hover:bg-[#be123c] hover:text-white transition">
                + Ajouter une ligne
              </button>
            </div>
            ...
            <div className="space-y-2">
              {data.lignes.map((l) => (
                <div key={l.id} className="grid gap-1.5 items-center" style={{ gridTemplateColumns: "1fr 65px 75px 85px 85px 28px" }}>
                  ...
                  <button type="button" onClick={() => removeLigne(l.id)} disabled={data.lignes.length === 1} className="flex items-center justify-center w-7 h-7 rounded-lg text-[#c0a0ac] hover:text-[#be123c] hover:bg-[#fdf2f5] transition disabled:opacity-30">✕</button>
                </div>
              ))}
            </div>
          ...
          <div className="grid grid-cols-3 gap-2 pb-10">
            <button type="button" onClick={handleConvertirFacture} className="py-3 rounded-xl border-2 border-[#e8d0d8] text-[13px] font-semibold text-[#6a3a4a] hover:border-[#be123c] hover:text-[#be123c] transition">🧾 → Facture</button>
            <button type="button" onClick={handleSave} className="py-3 rounded-xl border-2 border-[#e8d0d8] text-[13px] font-semibold text-[#6a3a4a] hover:border-[#be123c] transition">
              {saved ? "✓ Sauvegardé" : "Sauvegarder"}
            </button>
            <button type="button" onClick={handlePrint} className="py-3 rounded-xl bg-[#be123c] text-white text-[13px] font-bold hover:bg-[#e11d48] transition shadow-[0_6px_20px_rgba(190,18,60,0.3)]">📄 Générer PDF</button>
          </div>
        </div>
