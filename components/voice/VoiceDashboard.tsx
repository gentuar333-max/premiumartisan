...
function CallRow({
  ...
  const accentColor = call.urgent ? "#c00" : call.isnew ? "#16a34a" : "transparent"
  return (
    <div
      onClick={onClick}
      style={{
  ...
  return (
    <div style={{ padding: "16px 20px", display: "flex", flexDirection: "column", gap: 16, overflowY: "auto" }}>
      ...
      <div style={{ borderBottom: "2px solid #111", paddingBottom: 11 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 7, flexWrap: "wrap", marginBottom: 3, fontFamily: "'Playfair Display', serif", fontSize: 18, fontWeight: 800, letterSpacing: "-.02em", color: "#111" }}>
          {call.name}
          {call.urgent && <Badge type="urg" label="⚠ Urgent" />}
          <Badge type={ST[call.status].cls} label={ST[call.status].l} />
        </div>
        <div style={{ fontSize: 10, color: "#999", lineHeight: 1.8 }}>
          <strong>{fmtDt(call.dt)}</strong> · <strong>{SLOT[call.slot]}</strong>
          {" · "}
          {call.isnew
          ...
          <tbody>
            {[
              ...
              <tr key={row.k} style={{ borderBottom: "1px solid #ede9e0" }}>
                ...
                <td style={{ padding: "6px 10px", verticalAlign: "top" }}>{row.v}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <div>
        ...
        <div style={{ position: "relative" }}>
          <button
            ...
            style={{ width: "100%", padding: "7px 12px", background: "#fff", border: "1px solid #ddd", fontSize: 10, fontWeight: 700, textTransform: "uppercase", letterSpacing: ".06em", color: "#888", cursor: "pointer", textAlign: "left", fontFamily: "inherit", display: "flex", justifyContent: "space-between", alignItems: "center" }}
          >
            <span>{ST[call.status].l}</span>
            <span>▾</span>
          </button>
          {dropOpen && (
            <div style={{ position: "absolute", bottom: "100%", left: 0, right: 0, background: "#fff", border: "1px solid #ddd", zIndex: 99 }}>
              {(Object.keys(ST) as CallStatus[]).map((s) => (
                <div
                  key={s}
                  onClick={() => { onStatusChange(call.id, s); setDropOpen(false) }}
...
function SectionTitle({ children }: { children: React.ReactNode }) {
  return (
    <div style={{ fontSize: 8, fontWeight: 700, textTransform: "uppercase", letterSpacing: ".1em", color: "#bbb", display: "flex", alignItems: "center", gap: 6, marginBottom: 7 }}>
      {children}
...
export default function VoiceDashboard() {
  ...
  const list =
    ...
    : filter === "urg" ? calls.filter((c) => c.urgent)
    : calls
    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .vd-drop-item:hover { background: #f5f5f5 !important; color: #111 !important; }
        .vd-act:hover { opacity: .82; }
      `}</style>
      ...
      <div style={{ fontFamily: "'Inter', sans-serif", display: "flex", flexDirection: "column", height: "100vh", overflow: "hidden", background: "#fff", color: "#111" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", borderBottom: "1px solid #e8e4d8", background: "#fffef9", flexShrink: 0 }}>
          {stats.map((s, i) => (
            ...
            >
              <div style={{ fontFamily: "'Playfair Display', serif", fontSize: 20, fontWeight: 700, lineHeight: 1, color: filter === s.key ? "#fff" : (s.color ?? "#111") }}>
                {s.value}
              </div>
              <div style={{ fontSize: 9, color: filter === s.key ? "#aaa" : "#bbb", textTransform: "uppercase", letterSpacing: ".07em", marginTop: 3 }}>
                {s.label}
              </div>
            </div>
          ))}
        ...
        <div style={{ display: "grid", gridTemplateColumns: "minmax(140px,280px) 1fr", flex: 1, overflow: "hidden" }}>
          ...
          <div style={{ borderRight: "1px solid #e8e4d8", display: "flex", flexDirection: "column", overflow: "hidden" }}>
            ...
            <div style={{ overflowY: "auto", flex: 1 }}>
              {list.length === 0 && (
                <div style={{ padding: 24, textAlign: "center", color: "#ddd", fontSize: 11, fontStyle: "italic" }}>Aucun appel</div>
              )}
