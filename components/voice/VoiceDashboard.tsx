              ...
              <div style={{ textAlign: "center", padding: "40px 20px" }}>
                {subscription?.twilio_number ? (
                  <div style={{ maxWidth: 360, margin: "0 auto" }}>
                    <div style={{ fontSize: 13, color: "#9CA3AF", marginBottom: 20, lineHeight: 1.6 }}>Aucun appel pour le moment</div>
                    <div style={{ background: "#F8FAFF", border: "1px solid #DBEAFE", borderRadius: 14, padding: "20px 18px" }}>
                      <div style={{ fontSize: 11, color: "#3B82F6", textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 6 }}>Votre numero Marie</div>
                      <div style={{ fontSize: 20, fontWeight: 800, color: "#1D4ED8", letterSpacing: "0.03em", marginBottom: 12 }}>{subscription.twilio_number}</div>
                      <div style={{ fontSize: 12, color: "#6B7280", lineHeight: 1.7, marginBottom: 16 }}>
                        Partagez ce numero a vos clients. Marie repond a votre place, collecte leurs informations et vous envoie un rapport chaque soir a 17h.
                      </div>
                      <div style={{ display: "flex", flexDirection: "column", gap: 8, textAlign: "left" }}>
                        {["Donnez ce numero a vos clients", "Marie repond si vous etes indisponible", "Rapport SMS chaque soir a 17h"].map((tip, i) => (
                          <div key={i} style={{ display: "flex", alignItems: "center", gap: 10, fontSize: 12, color: "#374151" }}>
                            <span style={{ width: 18, height: 18, borderRadius: "50%", background: "#EFF6FF", color: "#3B82F6", fontSize: 10, fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>{i + 1}</span>
                            {tip}
                          </div>
                        ))}
                      </div>
                  ...
                  <div style={{ maxWidth: 320, margin: "0 auto" }}>
                    ...
                    <div style={{ fontSize: 14, fontWeight: 600, color: "#374151", marginBottom: 8 }}>Numero en cours d activation</div>
                    <div style={{ fontSize: 12, color: "#9CA3AF", lineHeight: 1.7, marginBottom: 20 }}>
                      Votre numero Marie est en cours de configuration. Cela prend generalement moins d une minute.
                    </div>
                    <div style={{ fontSize: 11, color: "#D1D5DB", lineHeight: 1.6 }}>
                      Si le numero n apparait pas apres quelques minutes, contactez-nous a <span style={{ color: "#3B82F6" }}>contact@premiumartisan.fr</span>
                    </div>
                  </div>
                ) : (
                  <div style={{ color: "#9CA3AF", fontSize: 13 }}>Aucun appel pour le moment</div>
                )}
              </div>
            ) : calls.map(call => {
